...
**A:** By default, JAX-RS treats Resource classes as "per-request," meaning a new instance is constructed for each incoming HTTP request. Because numerous instances interact with the storage layer simultaneously, shared in-memory data structures must be thread-safe. To prevent data loss or race conditions (e.g. concurrent modifications to lists), we utilize a centralized Singleton `DataStore` leveraging `ConcurrentHashMap` for primary entity mapping. Furthermore, we employ `CopyOnWriteArrayList` for nested collections like `sensorIds` and `readings` to ensure that iterator operations remain safe during concurrent updates from multiple threads.
...
**A:** HATEOAS (Hypermedia as the Engine of Application State) enables "self-describing messages" where the API response provides the client with the roadmap of what they can do next. This is considered a hallmark of advanced REST because it transforms the API from a static set of endpoints into a dynamic state machine. For client developers, this reduces hard-coding of URIs and allows the server to change its internal structure or URI scheme without breaking clients—developers simply follow the links provided in the response metadata rather than relying on out-of-date static documentation.
...
**A:** Using JAX-RS filters for cross-cutting concerns like logging is advantageous as it follows the DRY (Don't Repeat Yourself) principle. By implementing `ContainerRequestFilter` and `ContainerResponseFilter`, we centralize observability logic. This ensures that every entry and exit point—including newly added resources or built-in error responses (like 405 Method Not Allowed)—is automatically logged without manual intervention. This decoupling improves maintainability and guarantees a uniform audit trail across the entire application state.
